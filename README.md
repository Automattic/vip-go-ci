...
`vip-go-ci` is a PHP command-line program intended to scan commits pushed to pull requests on GitHub. It will call external scanners to perform the actual scans, then collects the issues found, and posts them as comments and reviews to the GitHub pull requests associated with the commits. `vip-go-ci` aims to unify the interface for users, displaying issues found by scanners in a uniform way, and to automate the analysis of code. Further, it will try to avoid posting multiple times about issues found in earlier scans.
...
* [SVG scanner](https://github.com/Automattic/vip-go-svg-sanitizer). Intended to find suspcious attributes or external references.
...
Publishing a GitHub build status is supported by `vip-go-ci`.
...
`vip-go-ci` supports scanning the same commit multiple times. When new commits are added to a pull request, the latest commit can be scanned, and no action has to be taken with earlier feedback generated by it.
...
First install the utilities using `tools-init.sh`. Then clone the repository that should be scanned, check out the branch that should be scanned and ensure it is update to date.
...
While running, `vip-go-ci` will output log of its actions. Here is an example -- note that output you see can differ substaintially:
