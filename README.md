...
Then the hash has to be calculated. First, obtain a `.tar.gz` archive for the release from GitHub. The download URL for `WordPress-Coding-Standards` is: `https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/archive/VERSION.tar.gz` -- simply replace `VERSION` with the latest version. Then run the `sha1sum` UNIX utility against the downloaded file. Any other compatibile tool can be used. 

For version 2.1.1 the hash is added as follows:
